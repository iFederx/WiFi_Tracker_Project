...
#define DEFAULT_SERVER_IP     "192.168.1.8"
...
void server_messanger(int socket);
void server_command(int socket);
...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            /* HERE create 2 process on Server
             * The first read the messages from the ESP32
             * The second send commands to him
             * */
            if((childpid=fork())<0)
            {
                printf("[!] Error: Task fork() failed");
...
void server_messanger(int socket){
    ...
    strcpy(buf, "ACCEPT\r\n");
    printf("[!] message ACCEPTED -> created\n");
    ...
    printf("[+] Synchronization phase Started\n");
    int repetition = 3;
    int rtt = 0;
...
void server_command(int socket){
    while(!(strcmp(buf,"CLOSE")==0))
    {
        ...
        //case of timestamp_sync required
        if(strstr(buf, "SYNC") != NULL){
            /* waiting the receive from the esp32*/
            if((len=recv(socket, rbuf, BUFLEN, 0))>0)
            {
                rbuf[len]='\0';
                if(strstr(rbuf,"SYNC") != NULL)
                    printf("[+] Synchronization phase Started\n");
            }else{
                printf("[!] Synchronization problem accured\n");
            }
